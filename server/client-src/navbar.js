  ...
  render() {
    ...
    const navbarLinks = navbarEntries.map((entry) => {
      if (currentLink.startsWith(entry.link)) {
        return <a key={entry.name} className={activeButtonClasses} href={entry.link}>{entry.name}</a>;
      } else {
        return <a key={entry.name} className={buttonClasses} href={entry.link}>{entry.name}</a>;
      }
    });
        ...
        <div className="container-fluid">
          <div className="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ol className="nav navbar-nav">
              <a key={'/'} className={homeButtonClasses} href="/">Melee Authority</a>
              {navbarLinks}
            </ol>
