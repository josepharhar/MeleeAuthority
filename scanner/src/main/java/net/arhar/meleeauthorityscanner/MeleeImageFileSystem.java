...
public class MeleeImageFileSystem {
  ...
  private void readFiles(MeleeImageFile isoFolder, long offset) throws IOException {
    ...
    while (currentOffset < offset) {
      ...
      MeleeImageFile file =
          new MeleeImageFile(isoFileName, fileSize, fileOffset, currentOffset, isFolder, null);
  ...
  private void readDOL() throws IOException {
    if (meleeFile.getFile().length() > 0x438L) {
      ...
      meleeFile.getFile().seek(meleeFile.getFile().getChannel().position() + 0x1cL);
      fileOff = meleeFile.getFile().readInt();
      pos = meleeFile.getFile().readInt();
      meleeFile.getFile().readInt();
      size = pos - fileOff;
      meleeFile.getFile().seek(meleeFile.getFile().getChannel().position() + 8L);
      meleeFile.getFile().readInt();
      MeleeImageFile startDol =
          new MeleeImageFile("Start.dol", size, fileOff, meleeFile.getFSTOffset(), false, null);
      cacheFile(startDol);
    }
  }
  ...
  private void readData(MeleeImageFile file) throws Exception {
    ...
    try {
      ...
      while (offset < totalLength) {
        ...
        long len = offset + fileSize < totalLength ? fileSize : totalLength - offset;
  ...
  private String getAbsoluteFileName(long stringOffset) throws IOException {
    String filename = "";
    if (meleeFile.getStringTableOffset() == 0) return null;
  ...
  public boolean replaceFile(MeleeImageFile file, byte[] data) throws IOException {
    if (meleeFile.isOpen()) {
      if (file.size != data.length) {
        System.out.println(
            "Length does not match!" + " File to Replace: " + file.size + " With: " + data.length);
        return false;
      }
    ...
    System.out.println(
        "File replace failed! iso size:" + data.length + " replace size:" + data.length);
    return false;
  }
